  ...
  const processConvert = () => {
    ...
    const processed = lines.map(line => {
      try {
        ...
        if (parts.length < 2) return { original: line, error: 'Invalid format' };
        ...
        const givenTitleSpaced = givenNameSyllables.map(s => capitalize(s)).join(' ');
        const format1 = `${givenTitleSpaced} ${surnameUpper} (${chineseName})`;

        const givenTitleJoined = givenNameSyllables.map(s => capitalize(s)).join('');
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
          <label className="block text-sm font-semibold mb-2 text-slate-600">
            Paste Data (Format: <code className="bg-slate-100 px-1 rounded text-pink-600">yameisong,宋亚梅</code>)
          </label>
          <textarea
          ...
          <div className="mt-3 flex justify-between items-center">
            <div className="text-sm text-slate-500">
              Total Count: <span className="font-semibold text-slate-700">{inputLines}</span>
            </div>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex flex-col">
              <div className="flex justify-between items-center mb-4">
                <div>
                  <h3 className="font-bold text-slate-700">Format 1: "Ya Mei SONG (宋亚梅)"</h3>
                  <p className="text-xs text-slate-500 mt-1">Total Count: <span className="font-semibold">{successCount}</span></p>
                </div>
                <button
                  onClick={() => handleCopy('format1')}
                  className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${
                    copiedIndex === 'format1' ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
                  }`}
                >
                  {copiedIndex === 'format1' ? <Check className="w-4 h-4" /> : <Copy className="w-4 h-4" />}
              ...
              <div className="bg-slate-50 rounded-xl border border-slate-100 p-4 max-h-[400px] overflow-y-auto space-y-2 font-mono text-sm">
                {results.map((res, i) => (
                  <div key={i} className={res.error ? "text-red-400 italic" : "text-slate-700"}>
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex flex-col">
              <div className="flex justify-between items-center mb-4">
                <div>
                  <h3 className="font-bold text-slate-700">Format 2: "Song YaMei"</h3>
                  <p className="text-xs text-slate-500 mt-1">Total Count: <span className="font-semibold">{successCount}</span></p>
                </div>
                <button
                  onClick={() => handleCopy('format2')}
                  className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${
                    copiedIndex === 'format2' ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
                  }`}
                >
                  {copiedIndex === 'format2' ? <Check className="w-4 h-4" /> : <Copy className="w-4 h-4" />}
